...
const DEFAULT_HERO = {
  leftBadge: 'Hello!', leftTitle: 'Strategist.',
  leftDesc: 'Performance marketing driving brand growth through creative campaigns.',
  rightBadge: 'Hello!', rightTitle: 'Analyst.',
  rightDesc: 'Data-driven decisions backed by deep analytics and performance tracking.',
};
...
export default function Hero() {
  ...
  useEffect(() => {
    fetch('/api/home').then(r => r.json()).then(d => { if (d.hero) setHero(d.hero); });
  }, []);
  ...
  return (
    <section ref={containerRef} style={{ height: '100vh', position: 'relative', overflow: 'hidden', paddingTop: '64px', cursor: 'ew-resize', userSelect: 'none' }}>

      <div style={{ position: 'absolute', inset: 0, background: '#111', zIndex: 0 }}>
        <img src="/nirp-real.png" alt="" style={imgStyle} draggable={false} />
      </div>

      <div style={{ position: 'absolute', inset: 0, zIndex: 1, clipPath: `inset(0 ${(100 - +lPct).toFixed(3)}% 0 0)`, background: '#FCF6F5' }}>
      ...
      {/* DIVIDER */}
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: `${lPct}%`, width: '2px', background: 'linear-gradient(to bottom, transparent, rgba(255,255,255,0.9) 20%, rgba(255,255,255,0.9) 80%, transparent)', zIndex: 10, transform: 'translateX(-1px)', pointerEvents: 'none' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '44px', height: '44px', borderRadius: '50%', background: '#fff', boxShadow: '0 2px 20px rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px', color: '#888' }}>⟺</div>
      </div>

      {/* Scroll hint */}
      <div style={{ position: 'absolute', bottom: '28px', left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', zIndex: 20, pointerEvents: 'none', animation: 'bounce 2s ease-in-out infinite' }}>
      ...
      <style>{`@keyframes bounce { 0%,100%{transform:translateX(-50%) translateY(0)}50%{transform:translateX(-50%) translateY(7px)} }`}</style>
    </section>
  );
}
